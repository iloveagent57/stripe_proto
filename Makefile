...
isort_check: ## check that isort has been run
	isort --check-only stripe_proto/

isort: ## run isort to sort imports in all Python files
	isort --recursive --atomic stripe_proto/

style: ## run Python style checker
	pylint --rcfile=pylintrc stripe_proto *.py
...
pii_check: ## check for PII annotations on all Django models
	DJANGO_SETTINGS_MODULE=stripe_proto.settings.test \
...
dummy_translations: ## generate dummy translation (.po) files
	cd stripe_proto && i18n_tool dummy
...
open-devstack: ## open a shell on the server started by start-devstack
	docker exec -it stripe_proto /edx/app/stripe_proto/devstack.sh open

pkg-devstack: ## build the stripe_proto image from the latest configuration and code
	docker build -t stripe_proto:latest -f docker/build/stripe_proto/Dockerfile git://github.com/openedx/configuration

detect_changed_source_translations: ## check if translation files are up-to-date
	cd stripe_proto && i18n_tool changed
...
app-shell: # Run the app shell as root
	docker exec -u 0 -it stripe_proto.app bash

db-shell: # Run the app shell as root, enter the app's database
	docker exec -u 0 -it stripe_proto.db mysql -u root stripe_proto
...
%-attach:
	docker attach stripe_proto.$*

github_docker_build:
	docker build . -f Dockerfile --target app -t openedx/stripe_proto

github_docker_tag: github_docker_build
	docker tag openedx/stripe_proto openedx/stripe_proto:${GITHUB_SHA}

github_docker_auth:
	echo "$$DOCKERHUB_PASSWORD" | docker login -u "$$DOCKERHUB_USERNAME" --password-stdin

github_docker_push: github_docker_tag github_docker_auth ## push to docker hub
	docker push 'openedx/stripe_proto:latest'
	docker push "openedx/stripe_proto:${GITHUB_SHA}"
